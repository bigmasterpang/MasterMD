...
- 扩展语法：**提示块**（`> [!NOTE] 标题`，支持 note/tip/important/warning/caution 等 10 种类型）、**上标 `x^2^`**、**下标 `H~2~O`**、数学公式、Mermaid 图表
...
- 配色方案：蓝 / 紫 / 绿 / 橙 / 红
- 快捷键自定义（9 个可绑定项）+ 冲突提示
- **版本更新检查**：启动时自动检查 GitHub 最新 Release（可在设置中关闭），发现新版本时状态栏提示并可查看更新说明；设置页可手动「检查更新」
...
- 版本来源：GitHub 仓库 `bigmasterpang/MasterMD` 的最新 Release（无 Release 时回退比较最新 tag）
- 启动 2.5 秒后自动静默检查（设置 → 版本更新 可关闭）；发现新版本时状态栏出现「发现新版本」入口
- 设置页「检查更新」按钮可手动检查，弹窗展示版本号、发布时间与更新说明，可一键打开发布页面
