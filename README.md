...
在「关于 MasterMD」中连续点击左上角图标 **7 次**，可解锁隐藏文字冒险 **「Markdown 地牢」**：
整个游戏都用 Markdown 演出（引用块做旁白、表格做属性、任务列表做背包、代码块做战斗日志、==高亮== 标记弱点），收集语法碎片、击败「格式混乱之兽」即可通关。

### 隐藏玩法 / 彩蛋 🥚

**1. Markdown 江湖（文字武侠，工具栏「江湖」按钮进入）**

内置的大型隐藏玩法，全部内容用 Markdown 呈现（表格=属性与背包、任务列表=进度、引用块=剧情、代码块=战斗记录）：

| 系统 | 说明 |
| --- | --- |
| 六大地图 | 青石镇 → 落霞谷 → 断魂崖 → 烟雨楼 → 大漠孤城 → 无名剑冢，按主线逐级解锁 |
| 数值与境界 | 1–60 级，每 10 级一个境界（初窥门径 → 出神入化）；攻击/防御/身法/气血/内力/暴击/吸血 |
| 装备掉落 | 6 部位 × 6 品质（凡品→神兵），随机词条，强化 +1~+10，背包内自动显示与当前装备的差值 |
| 门派 | 青城剑派 / 少林 / 唐门 / 逍遥谷，四套技能树（4 层，含绝学），消耗贡献与银两学习并升级 |
| 任务 | 主线 12 章 + 每图支线 + 日常；奖励经验、银两、贡献、保底品质装备 |
| 秘境 | 六处副本，3 层推进，可「继续深入」或「携宝撤退」，每日限次 |
| 挂机 | 每 5 秒一场自动战斗，可配置自动用药/拾取；离线按同速率补算，上限 12 小时（超出减半） |
| 成长记录 | 12 项成就、8 个称号，进度自动存档 |

数值经自动化模拟校准：0 战死完成 1→60 级全流程（约 3900 场战斗），精英与 Boss 需要装备与药品支撑。

**2. Markdown 地牢（关于面板连点图标 7 次解锁）**

小型文字冒险：随机遭遇战斗/宝箱/篝火/书架，收集语法碎片作为技能，击败第 5 层的「格式混乱之兽」通关。
